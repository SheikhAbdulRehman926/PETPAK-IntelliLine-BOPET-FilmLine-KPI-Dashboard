...
# =========================
st.set_page_config(
    page_title="PETPAK IntelliLine — BOPET FilmLine KPI Dashboard",
    page_icon="PETPAK-02.png",
    layout="wide",
...
DEV_ROLE  = "Developer / Artificial Intelligence Engineer • Internship at PETPAK"
DEV_LINK  = "https://www.linkedin.com/in/sheikh-abdul-rehman-a14a982ab"

FOOTER_LEFT        = "© 2025 • PETPAK IntelliLine — BOPET FilmLine KPI Dashboard  • Version-01 "
...
def _logo_b64(paths: Tuple[str, ...] = (
    "petpak-01.png", "PETPAK-01.png", "./petpak-01.png", "./PETPAK-01.png", "/mnt/data/petpak-01.png", "/mnt/data/PETPAK-01.png"
)) -> Optional[str]:
    for p in paths:
        if os.path.exists(p):
            with open(p, "rb") as f:
...
def _logo_bytes() -> Optional[bytes]:
    for p in ("petpak-01.png", "PETPAK-01.png", "./petpak-01.png", "./PETPAK-01.png", "/mnt/data/petpak-01.png", "/mnt/data/PETPAK-01.png"):
        if os.path.exists(p):
            with open(p, "rb") as f:
                return f.read()
    return None
...
def _build_pdf(figs: List[go.Figure], width=1600) -> Tuple[Optional[bytes], Optional[str]]:
    """
    PDF theme: dark background, PETPAK logo on each page, footer bar with PETPAK + clickable SARKS.
...
def _build_ppt(figs: List[go.Figure], title: str, narrative_lines: List[str]) -> Tuple[Optional[bytes], Optional[str]]:
    """
    PPT theme: dark background + PETPAK logo on every slide (no footer bar).
    Slides:
      • Title slide
    ...
    try:
        ...
        def _logo_bytes() -> bytes:
            candidates = [
                "PETPAK-01.png",
                "./assets/PETPAK-01.png",
                "./static/PETPAK-01.png"
            ]
        ...
        BRANDC  = RGBColor(0xF5, 0x60, 0x23)  # PETPAK orange
        ...
        ps = stf.paragraphs[0]
        ps.text = "BOPET FilmLine KPI Dashboard • PETPAK IntelliLine"
        ...
        for i, fig in enumerate(figs, start=1):
            ...
            default_bullets = [
                "Monthly trend with PETPAK dark theme.",
...
def _logo_bytes() -> bytes:
    ...
    - Reads PETPAK-01.png if available
...
def _download_row(tab_name: str, csv_df: Optional[pd.DataFrame], figs: List[go.Figure], narrative_lines: List[str]):
    ...
    with col3:
        ppt_bytes, ppt_err = _build_ppt(figs, f"{tab_name} — PETPAK IntelliLine", narrative_lines)
        if ppt_bytes:
            st.download_button(
                f"⬇ PPT — {tab_name}",
                data=ppt_bytes,
...
def render_about_developer_page():
    ...
    No logo, neutral text; subtle brand gradient in the hero.
    """
    import textwrap
    ...
    <div class="about-wrap">
      <section class="about-hero">
        ...
        <div class="about-sub">PETPAK • IntelliLine • BOPET FilmLine KPI Dashboard</div>

        <div class="badge-row">
          <span class="badge">Sheikh Abdul Rehman bin Khalid Sharif — SARKS</span>
          <span class="badge-muted">{DEV_ROLE}</span>
        </div>
      ...
      <section class="about-grid">
        <div class="card">
          ...
          <ul>
            <li><b>Annual KPI dashboard</b> — monthly trends for production, waste, downtime, OEE, quality.</li>
            <li><b>Breakups</b> — deep-dives for waste and downtime with factors, Pareto, shares.</li>
            <li><b>Forecast</b> — next-month estimate with a simple uncertainty band.</li>
            <li><b>Exports</b> — CSV, PDF, PPT for exec-ready sharing.</li>
          </ul>
        </div>
        ...
        <div class="card">
          <h3>Highlights</h3>
          <ul>
            <li>Enterprise dark theme aligned to PETPAK branding.</li>
            <li>Interactive analytics (Plotly) and pro exports (ReportLab, python-pptx).</li>
            <li>Validation checks for totals vs. factors to avoid reporting mistakes.</li>
          </ul>
        </div>
        ...
        <div class="card">
          ...
          <p>Design system: PETPAK orange <code>{BRAND}</code> on slate backgrounds.</p>
        </div>

        <div class="card">
          <h3>Credits</h3>
          <p>Thanks to PETPAK mentors and the IntelliLine team for guidance and reviews.</p>
        </div>
      </section>
    ...
    # Render as HTML (never escapes like Markdown can)
...
if "petpak_df" not in st.session_state:
    st.session_state.petpak_df = pd.DataFrame()
...
# =========================
# Sidebar THEME (PETPAK gradient + brand colors)
# =========================
st.markdown(
    f"""
    <style>
      ...
      /* Subtle PETPAK logo watermark inside sidebar */
      section[data-testid="stSidebar"]::before {{
        content: "";
        position: absolute; 
        inset: 0;
...
if workbook_file is not None:
    ...
    if sheets:
        ...
        if kpi_sn and kpi_sn in sheets:
            st.session_state.petpak_df = clean_header_df(sheets[kpi_sn].copy())
        elif len(sheets) == 1:
            only_df = list(sheets.values())[0]
            st.session_state.petpak_df = clean_header_df(only_df.copy())
...
# Working copies
df = st.session_state.petpak_df.copy()
...
_ppk_logo64 = None
try:
    with open("PETPAK-02.png", "rb") as _f:
    ...
    st.markdown(
        ...
        <div style="
          ...
          <div style="
            ...
            <span style="color:{TEXT};">PETPAK</span>
            {"<img src='data:image/png;base64," + _ppk_logo64 + "' style='height:1.6em; width:auto; vertical-align:middle;'/>" if _ppk_logo64 else ""}
            <span>&nbsp;&middot;&nbsp; IntelliLine &middot; BOPET FilmLine KPI Dashboard</span>
          </div>
        </div>
        """,
        unsafe_allow_html=True
...
<div class="modal-overlay">
  <div class="modal-card">
    ...
    <p>This model was developed by <b>Sheikh Abdul Rehman</b> during his internship at <b>PETPAK</b>. 
       It follows PETPAK’s visual identity (logo, colors and typography) and presents factory KPIs 
